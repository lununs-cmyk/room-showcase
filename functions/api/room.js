...
export async function onRequestGet(context){
  try{
    ...
    const [messages,images]=await Promise.all([
      ...
      context.env.DB.prepare(`SELECT id,image_name,image_type,image_size,sort_order FROM room_images WHERE room_id=? ORDER BY sort_order ASC,id ASC LIMIT 5`).bind(result.roomId).all()
    ]);
    ...
    return json({success:true,room:result.room,images:mapped,messages:messages.results||[]});
